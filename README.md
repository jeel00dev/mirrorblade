...
- **Score and shards** — every unique mirrored cell placed is worth 10 points. Completed lines give 100 points each plus 100 for every pair cleared together. Normal run shards come directly from each clear: Single 1, Double 4, Triple 9, four-line Max 16; rows and columns count equally. They are added to persistent currency when the run ends.
...
- **Placement deadline** — every run starts with 30 seconds to place a polygon. The window follows score progression down to a 10-second minimum. Its last five seconds show a coral board-rim orbit and `5, 4, 3, 2, 1`; any valid placement hides the warning and resets the full current window. Guided onboarding is untimed.
...
- `src/game/` — `BoardState`, `MirrorRules`, `PlacementSystem`, `ClearResolver`, `BladeCutter`, `Piece` (rotation, symmetry, orientations), `PieceLibrary` (rated shapes), `PieceGenerator` (director-weighted / daily fixed), `DifficultyDirector`, `Contracts`, `PrecisionCells`, `Tray`, `MoveAnalyzer` (rotation-aware), `ScoreSystem`, `ComboSystem`, `SkillEvents`, `BladeEnergy` (rising recharge cost), `Overdrive`, `PlacementDeadline`, `Fracture`, `RunClock`, `DailyMode`, `SeededRandom`.
...
- Vitest: mirror rules, placement, clearing, cutting, rotation (90/180/270/360, normalization, collision, mirrored placement, fragments, orientation-aware analysis), blade energy (gains, exact 100, overflow, single forge, cap, banking, redemption), Overdrive, the 30→10 second placement deadline (curve, reset, warning, expiry, clock gates), Fracture (fairness gates, warning → active, reset, escape, cooldown, clutch threshold, hidden-tab pause, timeout), scoring, generation (deterministic, board-independent daily), save migration and inventory, and the game-over cinematic.
- Playwright: mouse and touch placement, rotation, cutting, onboarding, resize recovery, deterministic score and shards including no-clear zero payout, energy, Overdrive, placement-deadline warning/reset/floor/timeout, Fracture lifecycle, hidden/menu timer freezes, every screen, settings, Shop equipment, deferred navigation, 16-viewport bounds / overlap / overflow, block-colour parity, and the game-over cinematic.
