...
export class BladeScene {
  ...
  public mount(container: HTMLElement, options: { hero?: boolean; travel?: boolean; pose?: 'upright' | 'dock' | 'showcase' } = {}): void {
    ...
    this.showcaseTime = 0;
    this.group.rotation.set(0.10, 0.32, this.tiltFor());
  ...
  private resize(): void {
    ...
    const fill = this.pose === 'showcase' ? 0.9 : this.hero ? 0.92 : 0.9;
  ...
  private tiltFor(): number {
    return this.pose === 'upright' ? -0.12 : this.pose === 'showcase' ? -1.08 : -0.55;
  }
