...
## 2026-09-12 — Responsive dynamic tray and Home control cleanup

Files: `src/render/{Layout,TrayView,PiecePreviewLayout}.ts`, `src/ui/GameplayView.ts`, `src/input/PointerController.ts`, `src/core/Game.ts`, `src/styles/{tokens,base,gameplay,katanas}.css`, `src/ui/screens/HomeScreen.ts`, debug bridge/types, unit and Playwright coverage, QA tooling/captures and responsive research/audit documentation.
